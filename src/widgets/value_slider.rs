...
impl<Message: Clone> field_editor::FieldHost<Message> for ValueSlider<Message> {
    fn text_size(&self) -> f32 {
        self.text_size
    }

    fn buffer(tree: &Tree) -> String {
        tree.state.downcast_ref::<State>().mode.buffer().to_owned()
    }

    fn set_buffer(tree: &mut Tree, buffer: String) {
        if let Mode::Editing { buffer: slot, .. } = &mut tree.state.downcast_mut::<State>().mode {
            *slot = buffer;
        }
    }

    fn take_focus_request(tree: &mut Tree) -> Option<String> {
        match &mut tree.state.downcast_mut::<State>().mode {
            Mode::Editing {
                buffer,
                needs_focus,
            } if *needs_focus => {
    ...
    fn filter(&self, input: String) -> String {
        field_editor::filter_number(&input, self.allows_decimal(), self.allows_minus(), 8)
    }

    fn publish_buffer(&self, buffer: &str, shell: &mut Shell<'_, Message>) {
        ValueSlider::publish_buffer(self, buffer, shell)
    }

    fn commit(&self, tree: &mut Tree, shell: &mut Shell<'_, Message>) {
        ValueSlider::commit(self, tree, shell)
    }
}
